...
/// Canonical v1CountrySpecs operation model declarations
public struct GetCountrySpecsResponse: Codable {
    ...
    /// `list`.
    public var object: GetCountrySpecsResponseObject
...
public extension GetCountrySpecsResponse {
    init(from decoder: Decoder) throws {
        ...
        try sdkValidatePattern("url", url, sdkPatternee6563d5db9e)
    }
}

public extension GetCountrySpecsResponse {
    init(data: [CountrySpec], hasMore: Bool, object: GetCountrySpecsResponseObject, url: String) throws {
        (self.data, self.hasMore) = (data, hasMore)
        (self.object, self.url) = (object, url)
        try validateLength("url", self.url, min: nil, max: 5000)
        try sdkValidatePattern("url", self.url, sdkPatternee6563d5db9e)
    }
}
