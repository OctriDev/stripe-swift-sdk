...
/// Canonical v1ApplePayDomains operation model declarations
public struct GetApplePayDomainsResponse: Codable {
    ...
    /// `list`.
    public var object: GetApplePayDomainsResponseObject
...
public extension GetApplePayDomainsResponse {
    init(from decoder: Decoder) throws {
        ...
        try sdkValidatePattern("url", url, sdkPatterneeafe0fcb834)
    }
}

public extension GetApplePayDomainsResponse {
    init(data: [ApplePayDomain], hasMore: Bool, object: GetApplePayDomainsResponseObject, url: String) throws {
        (self.data, self.hasMore) = (data, hasMore)
        (self.object, self.url) = (object, url)
        try validateLength("url", self.url, min: nil, max: 5000)
        try sdkValidatePattern("url", self.url, sdkPatterneeafe0fcb834)
    }
}
