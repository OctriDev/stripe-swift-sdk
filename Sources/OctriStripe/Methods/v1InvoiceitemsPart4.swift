...
public extension V1InvoiceitemsMethods {
    ...
    static func deleteInvoiceitemsInvoiceitem(
        config: ClientConfig,
        invoiceitem: String
    ) async throws -> DeletedInvoiceitem {
        try validateLength("invoiceitem", invoiceitem, max: 5000)

        return try await (sdkRequest(
            "DELETE",
            ["/v1/invoiceitems/", sdkEncodePathSegment(sdkWireString(invoiceitem))].joined(),
            config: config,
            decoder: .json,
    ...
    static func getInvoiceitemsInvoiceitem(
        config: ClientConfig,
        invoiceitem: String,
        expand: [String]?
