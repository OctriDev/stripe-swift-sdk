...
extension V1LinkedAccountsMethods {
    ...
    public static func getLinkedAccountsAccount(config: ClientConfig, account: String, expand: [String]?) async throws -> FinancialConnectionsAccount {
        try validateLength("account", account, max: 5000)

        return try (await sdkRequest("GET", ["/v1/linked_accounts/", sdkEncodePathSegment(sdkWireString(account))].joined(), config: config, query: [
            SdkQueryParameter("expand", values: expand, style: "deepObject", explode: true),
        ], decoder: .json, operationId: "GetLinkedAccountsAccount")).data
    }
}
