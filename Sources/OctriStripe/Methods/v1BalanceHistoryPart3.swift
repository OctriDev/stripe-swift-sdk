...
extension V1BalanceHistoryMethods {
    ...
    public static func getBalanceHistoryId(config: ClientConfig, id: String, expand: [String]?) async throws -> BalanceTransaction {
        try validateLength("id", id, max: 5000)

        return try (await sdkRequest("GET", ["/v1/balance/history/", sdkEncodePathSegment(sdkWireString(id))].joined(), config: config, query: [
            SdkQueryParameter("expand", values: expand, style: "deepObject", explode: true),
        ], decoder: .json, operationId: "GetBalanceHistoryId")).data
    }
}
