...
public extension V1IssuingCardsMethods {
    ...
    static func getIssuingCardsCard(config: ClientConfig, card: String, expand: [String]?) async throws -> IssuingCard {
        try validateLength("card", card, max: 5000)

        return try await (sdkRequest(
            "GET",
            ["/v1/issuing/cards/", sdkEncodePathSegment(sdkWireString(card))].joined(),
            config: config,
            query: [
